...
class PagesController extends AppController
{
    ...
    public function chapterView($idChap = null): ?Response
    {
        ...
        // save session
        $listReaded = [];
        $listReaded = $this->Session->read('list-readed');
        $isSaved = false;
        if ($listReaded && sizeof($listReaded) > 0) {
            foreach ($listReaded as $key => $readed) {
                if ($readed['tale_id'] == $chap["tale_id"]) {
                    $isSaved = true;
                }
            }
        }
        if (!$isSaved) {
            $newReaded['tale_id'] = $chap["tale_id"];
            $newReaded['chap_id'] = $idChap;
            $listReaded[] = $newReaded;
            $this->Session->write('list-readed', $listReaded);
        }
        echo "--------------------------Debug: " .json_encode( $listReaded, JSON_UNESCAPED_UNICODE);
